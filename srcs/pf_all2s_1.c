...
int	ft_pf_f2s(t_printf *f, char *str)
{
	...
	ft_pf_nbr_fega_exp_flags(f);
	ft_pf_nbr_any_signal(f, &arg);
	i = ft_pf_nbr_fega_exp_value(f, arg_arr, str, &arg);
	if ((*f).at == 'f')
		i = ft_pf_nbr_fega_mantissa(f, i, arg_arr, str);
	else if ((*f).at == 'a' || (*f).at == 'A' || \
				(*f).at == 'e' || (*f).at == 'E')
	{
		i += ft_pf_nbr_fega_mantissa(f, 1, arg_arr, str + i);
		(*f).f_hash = 1;
	}
	free(arg_arr);
	return (i);
...
/// @return		the number pf chars of the result
int	ft_pf_nbr_fega_exp_value(t_printf *f, char *arg_arr, char *str, void *arg)
{
	int		i;
	char	base;
